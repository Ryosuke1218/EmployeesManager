...
public class EditMgr extends SelectMgr{
	...
	public void showMenu(){
		System.out.println("社員情報を更新します。更新対象の社員の検索条件を指定してください。");
	...
	public void processEmployeeID() {
		...
		try {
			...
			if(maxVal == minVal) {
				...
				this.condition = "WHERE employee_id = "+maxVal;
			}else {
				sql = "SELECT * FROM employees WHERE employee_id >="+ minVal + " AND employee_id <="+ maxVal;
				this.condition = "WHERE employee_id >="+ minVal + " AND employee_id <="+ maxVal;
			}
	...
	public void processFirstName() {
		...
		try {
			...
			this.condition =  "WHERE first_name LIKE '%"+name+"%'";
	...
	public void processHireDate() {
		...
		try {
			...
			if(maxVal == minVal) {
				...
				this.condition = "WHERE hire_date = '"+maxVal+"'";
			}else {
				sql = "SELECT * FROM employees WHERE hire_date >= "+ minVal  + "  AND hire_date <= "+ maxVal;
				this.condition = "WHERE hire_date >= "+ minVal  + "  AND hire_date <= "+ maxVal;
			}
	...
	public void processEdit() {
		System.out.println("新しい給料を入力してください");
		InputStreamReader isr = new InputStreamReader(System.in);
		BufferedReader br = new BufferedReader(isr);
		try {
			//入力値を受け取る
			...
			if(this.checkDigit(str)) {
				//文字列を数値に変換する
				
				int salary = Integer.valueOf(str).intValue();
				
				//SQL文を発行する
				StringBuffer buf = new StringBuffer("UPDATE employees set salary=");
				buf.append(salary + " " + this.condition);
				String sql = buf.toString();
				BaseDB db = new BaseDB();
				//SQL実効
				int cnt = db.updateSQL(sql);
				if(cnt > 0) {
					System.out.println(cnt + "件更新されました。");;
				}else {
